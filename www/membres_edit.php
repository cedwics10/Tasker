<?php
require "../include/inc_config.php";
//applique la fonction mysqli_real_escape_string (protection contre l'injection SQL) à tous les élements de $_POST
$_POST = array_map("mres", $_POST);
extract($_POST);

if (isset($btsubmit)) {
    //si reception de données du formulaire
    if ($_id==0) 
        //création d'un enregistrement
        $sql="insert into membres values ('','$pseudo','$mdp','$photo','$role')";
    else
        //maj d'un enregistrement
        $sql = "update membres set pseudo='$pseudo',mdp='$mdp',photo='$photo',role='$role' where _id=$_id";

    //exécution de la requete insert/update puis redirection
    if (mysqli_query($link, $sql))
        header("location:membres_list.php");
    else
        echo mysqli_error($link);
        
} else {
    //récupération de l'identifiant de l'enregistrement à éditer
    $id = intval($_GET["id"]);
    if ($id > 0) {
        //edition d'un enregistrement existant
        $sql = "select * from membres where _id=$id";
        $resultat = mysqli_query($link, $sql);
        $ligne = mysqli_fetch_assoc($resultat);
        if ($ligne === false)
            echo mysqli_error($link);
        //applique lafonction htmlentities (protection contre l'injectrion javascript/HTML) à toutes les valeurs
        $ligne=array_map("mhe",$ligne);
        extract($ligne);
    } else {
        //création d'un nouvel enregistrement
        $_id=0;
        $pseudo='';$mdp='';$photo='';$role='';
    }
}
?>
<!DOCTYPE html>
<html lang="fr">

<head>
    <?php require "../include/inc_head.php"; ?>
</head>

<body>
    <!-- lien de navigation pour lecteur d'écran -->
    <a href="#main" class="sr-only">aller au contenu principal</a>
    <!-- entete de page -->
    <header>
        <?php require "../include/inc_header.php"; ?>
    </header>
    <!-- menu de navigation -->
    <nav>
        <?php require "../include/inc_menu.php"; ?>
    </nav>
    <!-- contenu principal -->
    <main id="main">
        <h2>membres : edition</h2>
        <form method="post">
            <input type="hidden" name="_id" value="<?= $_id ?>">
            <p>
                <label>_id</label> : <?= $_id ?>
            </p>
            <p>
<label for='pseudo'>pseudo</label>
<input type='text' name='pseudo' id='pseudo' value='<?= $pseudo ?>'>
</p>

<p>
<label for='mdp'>mdp</label>
<input type='text' name='mdp' id='mdp' value='<?= $mdp ?>'>
</p>

<p>
<label for='photo'>photo</label>
<input type='text' name='photo' id='photo' value='<?= $photo ?>'>
</p>

<p>
<label for='role'>role</label>
<input type='text' name='role' id='role' value='<?= $role ?>'>
</p>


            <p>
                <input type="submit" name="btsubmit" value="Envoyer">
            </p>
        </form>
    </main>
    <!-- pied de page -->
    <footer>
        <?php require "../include/inc_footer.php"; ?>
    </footer>

</body>

</html>